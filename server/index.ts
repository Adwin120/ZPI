...
app.post('/Klient', [
  ...
  body('email').matches(/^[^@\s]+@[^@\s\.]+\.[^@\.\s]+$/).withMessage('Niepoprawny format adresu e-mail.'),
  body('adres').notEmpty().withMessage('Adres jest wymagany.'),
  body('nip').matches(/^\d{10}$/).withMessage('NIP musi mieć 10 cyfr.'),
  body('telefon').matches(/^\d{9}$/).withMessage('Telefon musi mieć 9 cyfr.'),
...
app.post('/Pracownik', [
  body('email').matches(/^[^@\s]+@[^@\s\.]+\.[^@\.\s]+$/).withMessage('Niepoprawny format adresu e-mail.'),
  body('haslo').isLength({ min: 8 }).withMessage('Hasło musi mieć przynajmniej 8 znaków.').matches(/^(?=.*[A-Z])(?=.*[!@#$%^&*()_+{}[\]:;<>,.?~\\/-])/).withMessage('Hasło musi zawierać dużą literę i znak specjalny.'),
  body('telefon').matches(/^\d{9}$/).withMessage('Telefon musi mieć 9 cyfr.'),
